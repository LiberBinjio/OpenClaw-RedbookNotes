@ALL
归档路径建议：2026-04-05 / 20260405-080836+AI技能开始卷稳定交付真正拉开差距的是谁能把Skill接进workflow / material files
...
- 归档要求：所有下游产物统一对齐到 `2026-04-05 / 20260405-080836+AI技能开始卷稳定交付真正拉开差距的是谁能把Skill接进workflow / material files`。

2. 来源状态与主题要求
- 一手优先入口必须写明：`https://www.newrank.cn/search/trend/skill`。
- 本轮已按要求优先采用 Newrank 趋势入口作为主题来源边界，但当前环境仍未取得 `search/trend/skill` 可复核的详情页正文、榜单明细、热度值、排名或互动字段。
...
- 在上述边界下，本轮只能使用已整理的 Newrank 邻近公开标题/摘要级线索，以及 `.role-sessions/Topic/TOPIC.md` 中已收口的趋势判断；不得把这些线索包装成 `search/trend/skill` 详情页原始事实。
- 本轮主选题统一为：`AI 技能开始卷稳定交付，真正拉开差距的是谁能把 Skill 接进 workflow`。
...
- `百度秒哒应用生成Skill上线，面向全球用户开放`（2026-03-17）：说明技能化调用已从内容辅助延伸到应用生成。
- `你和最会玩“龙虾”的人，可能只差这8个硬核Skills`（2026-03-23）：说明讨论重点已从安装转向能力扩展与 workflow 实用性。
...
- Topic 归档字段必须统一到：
  - `date`: `2026-04-05`
  - `timestamp`: `20260405-080836`
  - `archive_title`: `AI技能开始卷稳定交付真正拉开差距的是谁能把Skill接进workflow`
  - `archive_package`: `2026-04-05 / 20260405-080836+AI技能开始卷稳定交付真正拉开差距的是谁能把Skill接进workflow / material files`
...
- 标题优先方向：
  1. AI 技能开始卷稳定交付了，真正拉开差距的是 workflow
...
- Draft 归档字段必须统一到 `20260405-080836`，不得沿用旧时间戳。
- 不得公开发布，不得写成自动发布指令。
...
- Cover 归档字段必须统一到 `20260405-080836`。
...
- 检查 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260405-080836`。
- 检查最终产物是否仅用于预览、复审与手动发布，不自动公开发布。
- 如果下游仍沿用 `20260405-073728`、`20260405-052327`、`20260405-034733` 等旧归档包，必须优先指出并要求统一修正。
...
- 下游角色以本文件与 `.role-sessions/Topic/TOPIC.md` 为准继续产出；如有冲突，以本轮时间戳 `20260405-080836` 与本文件来源边界为准统一收口。
...
- 本轮引用边界以 `.role-sessions/Topic/TOPIC.md` 已整理的 Newrank 邻近公开线索为准；仅可作为标题/摘要级趋势线索使用，不能扩写成未验证正文事实。
