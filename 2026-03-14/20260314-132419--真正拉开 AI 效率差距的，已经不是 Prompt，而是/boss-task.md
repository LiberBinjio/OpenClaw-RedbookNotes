...
3. 已复核上一轮 Boss 留痕与当前下游产物，确认上一轮 Review 指出的两个核心缺口仍是本轮必须收口的重点：
   - `Draft/@DRAFT.md` 缺少一个显式标注的简短 `viral-copy breakdown` 区块；
   - `@TOPIC` 与 `Cover/@COVER` 的 archive metadata 未统一到 Boss 规定的归档包。
...
6. 已确认本轮统一 archive package 必须切换到当前 cycle：`2026-03-14 / 20260314-132419+AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用 / material files`。
7. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。
8. 已确认完成后需执行 Telegram 通知：`Done: Boss cycle 20260314-132419`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用`。
- 统一归档日期：`2026-03-14`。
- 统一归档时间戳：`20260314-132419`。
- 统一归档包：`2026-03-14 / 20260314-132419+AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-132419` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-132419`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-132419+AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-132419.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260314-132419`，避免 Topic / Draft / Cover / Review 落入不同归档包。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-132419.md`
- 发布动作：未执行
