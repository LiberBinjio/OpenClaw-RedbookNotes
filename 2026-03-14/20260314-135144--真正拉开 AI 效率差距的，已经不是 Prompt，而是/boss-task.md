...
3. 已复核上一轮 Boss 留痕，继续沿用单主题收口策略：围绕“AI 技能正在从会聊升级到会把流程稳定复用”组织 Topic / Draft / Cover / Review 的统一交付。
4. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
5. 已尝试再次检查 `https://www.newrank.cn/search/trend/skill` 的可访问详情状态，但本轮环境内置抓取工具返回模型不支持错误，未形成新的可验证详情。因此本轮只能继续沿用已留痕的稳定 Newrank 邻近线索，并明确记录详情缺失与 fallback 不可用状态。
6. 已确认本轮统一 archive package 必须切换到当前 cycle：`2026-03-14 / 20260314-135144+AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用 / material files`。
7. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。
8. 已确认完成后需执行 Telegram 通知：`Done: Boss cycle 20260314-135144`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用`。
- 统一归档日期：`2026-03-14`。
- 统一归档时间戳：`20260314-135144`。
- 统一归档包：`2026-03-14 / 20260314-135144+AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-135144` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会写提示词，转向能不能把一段流程拆清、跑顺、反复复用”，而不是泛泛讨论模型参数。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮未拿到可验证的深层详情；并且本轮环境内置抓取工具尝试失败，返回模型不支持错误。因此下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
...
6. **来源限制说明**
   ...
   - 本轮 WebFetch 工具尝试访问 Newrank 入口时返回模型不支持错误，这属于环境限制，不构成新的趋势详情证据。
...
- Draft 必须新增一个简短且显式标注的 `viral-copy breakdown` 区块，满足当前任务要求与上一轮 Review 的打回意见。
- 归档目录、时间戳、中文标题必须与本轮统一 package 完全一致。
- 不得公开发布，不得写成自动发布指令。

### Cover 要求
- 封面方向必须与统一中文标题一致，并支持归档目录下独立留存。
- 主标题建议突出：`AI技能开始进入流程复利时代`。
- 副标题建议突出：`不是会聊更久，而是流程能稳定复用`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-135144`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-135144+AI技能开始进入流程复利时代：真正拉开差距的，不是会聊，而是能把一套动作稳定复用`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-135144.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260314-135144`，避免 Topic / Draft / Cover / Review 落入不同归档包。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-135144.md`
- 发布动作：未执行
