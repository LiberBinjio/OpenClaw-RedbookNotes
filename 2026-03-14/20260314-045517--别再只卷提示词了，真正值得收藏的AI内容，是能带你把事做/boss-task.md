...
4. 已复核上一轮稳定留痕 `Boss/BOSS-CYCLE-20260314-042907.md`、`.role-sessions/Topic.prompt.txt`、`.role-sessions/Cover.prompt.txt`、`.role-sessions/Review.prompt.txt` 与 `Draft/@DRAFT.md`，沿用其已确认的 archive package 约束、来源边界与非公开发布边界。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 本轮未新增可验证的 Newrank 深层详情，因此仍需沿用稳定规则：优先写明 `https://www.newrank.cn/search/trend/skill` 为一手入口；若详情缺失，只能诚实记录限制，并基于已留痕的 Newrank 邻近公开线索继续收口，不扩写未核实事实。
7. 已确认本轮 archive package 必须统一为：`2026-03-14 / 20260314-045517+AI技能内容开始真正分层：用户真正愿意收藏的，不是一次聊爽，而是一套能重复跑通的工作流 / material files`。
8. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始真正分层：用户真正愿意收藏的，不是一次聊爽，而是一套能重复跑通的工作流`。
- 统一归档日期：`2026-03-14`。
- 统一归档包：`2026-03-14 / 20260314-045517+AI技能内容开始真正分层：用户真正愿意收藏的，不是一次聊爽，而是一套能重复跑通的工作流 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始真正分层：用户真正愿意收藏的，不是一次聊爽，而是一套能重复跑通的工作流”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-045517` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会写提示词，转向能不能把一段流程拆清、跑顺、反复复用”，而不是泛泛讨论模型参数。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮没有新增可验证的深层详情；下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
2. **`openclaw` / `openclaw skill` 邻近趋势线索**
   - 可沿用上一轮已留痕的稳定公开线索：`百度电商Skill登陆OpenClaw，五大能力全开放`（voice.newrank.cn，2026-02-13）。
...
- 副标题建议突出：`用户真正愿意收藏的，不是一次聊爽，而是一套能重复跑通的工作流`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-045517`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-045517+AI技能内容开始真正分层：用户真正愿意收藏的，不是一次聊爽，而是一套能重复跑通的工作流`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-045517.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260314-045517`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-045517.md`
- 发布动作：未执行
