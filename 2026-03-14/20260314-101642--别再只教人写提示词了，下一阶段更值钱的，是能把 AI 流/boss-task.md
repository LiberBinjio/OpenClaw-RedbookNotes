...
4. 已复核既有 Boss 留痕与归档包样式，确认本轮仍应输出一份可直接驱动 Topic / Draft / Cover / Review 的统一任务文件。
5. 已复核既有 topic package 样式，确认 Topic 必须显式写出 `Newrank 优先、详情受限、fallback skill 不可用` 的真实来源边界，并补齐 viral-copy breakdown 与 rewrite direction。
6. 当前会话可用 skills 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本轮不能执行，也不能伪造抓取结果。
7. 本轮未新增可验证的 `https://www.newrank.cn/search/trend/skill` 深层详情；因此只能把 Newrank 写成优先入口与检查对象，并沿用既有稳定 Newrank 邻近线索，明确写出详情缺失 / fallback 不可用状态。
8. 本轮统一 archive package 采用：`2026-03-14 / 20260314-095103+AI技能内容开始拼交付，不再拼谁写出了更长的提示词，而是谁把整套流程做成可复用成果 / material files`。
...
- 统一归档日期：`2026-03-14`。
- 统一归档包：`2026-03-14 / 20260314-095103+AI技能内容开始拼交付，不再拼谁写出了更长的提示词，而是谁把整套流程做成可复用成果 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-095103` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-095103`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-095103+AI技能内容开始拼交付，不再拼谁写出了更长的提示词，而是谁把整套流程做成可复用成果`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-095103.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一到 `20260314-095103`，避免 Topic / Draft / Cover / Review 沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-095103.md`
- 发布动作：未执行
