...
3. 已复核同日近邻留痕 `Boss/BOSS-CYCLE-20260314-110646.md` 与 `Boss/BOSS-CYCLE-20260314-055236.md`，沿用其已确认的来源边界、viral-copy 拆解要求、archive package 约束与非公开发布边界。
4. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
5. 已确认本轮 Topic 仍必须优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写进来源状态；但当前没有新增可验证的 Newrank 深层详情抓取结果，因此仍只能沿用已留痕的稳定 Newrank 邻近线索，并明确记录详情缺失与 fallback 不可用状态。
6. 已确认本轮 archive package 必须统一为：`2026-03-14 / 20260314-144419+AI技能内容开始真正分层：用户最后带走的不是一句提示词，而是一套能重复跑通的流程 / material files`。
7. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始真正分层：用户最后带走的不是一句提示词，而是一套能重复跑通的流程`。
- 统一归档日期：`2026-03-14`。
- 统一归档包：`2026-03-14 / 20260314-144419+AI技能内容开始真正分层：用户最后带走的不是一句提示词，而是一套能重复跑通的流程 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始真正分层：用户最后带走的不是一句提示词，而是一套能重复跑通的流程”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-144419` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- Hook：先抛出“很多人还在把 AI 当提示词工具，但更高传播价值的内容已经开始教人把整段流程带走”这种认知反差。
- 结构：先指出旧用法的局限，再展示工作流型内容为什么更容易被收藏，最后落到一条可照着走的执行路径。
- 冲突点：会写 prompt 不再足够，真正的差距转移到能否把选题、写稿、审稿、归档串成稳定闭环。
- 承诺：读者看完不是只得到一个观点，而是得到一套可模仿、可复查、可继续优化的内容生产方式。
- 证据：只能引用 Newrank 优先入口受限状态与已留痕的邻近公开线索，不能伪造详情页正文。
- CTA：引导读者对照自己的内容流程，判断自己是在生产“一次聊完”的结果，还是在沉淀“可重复跑通”的内容资产。

### Rewrite Direction
- 保留：爆款内容里“反常识开头 + 路径拆解 + 明确收益 + 可执行结尾”的结构模式。
- 改写：所有关键表达必须换成围绕“AI技能内容开始真正分层：用户最后带走的不是一句提示词，而是一套能重复跑通的流程”的当前主题表达，不复制任何原句。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-144419`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-144419+AI技能内容开始真正分层：用户最后带走的不是一句提示词，而是一套能重复跑通的流程`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-144419.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260314-144419`，避免 Topic / Draft / Cover / Review 沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-144419.md`
- 发布动作：未执行
