...
4. 已复核上一轮稳定留痕 `Boss/BOSS-CYCLE-20260314-045517.md` 与近邻 Boss cycle 写法，沿用其已确认的 archive package 约束、来源边界与非公开发布边界。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 已尝试检查 `https://www.newrank.cn/search/trend/skill`；当前在线抓取工具返回 `API Error 400`，因此本轮不能写成已成功取得 Newrank 详情，只能如实记录：优先入口已检查，但未获得可验证深层详情。
7. 已确认本轮 archive package 必须统一为：`2026-03-14 / 20260314-052157+AI技能内容开始拼可复用交付，真正能拉开差距的不是多会提问，而是能不能把整套流程稳定跑完 / material files`。
8. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始拼可复用交付，真正能拉开差距的不是多会提问，而是能不能把整套流程稳定跑完`。
- 统一归档日期：`2026-03-14`。
- 统一时间戳：`20260314-052157`。
- 统一归档包：`2026-03-14 / 20260314-052157+AI技能内容开始拼可复用交付，真正能拉开差距的不是多会提问，而是能不能把整套流程稳定跑完 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始拼可复用交付，真正能拉开差距的不是多会提问，而是能不能把整套流程稳定跑完”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-052157` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会写提示词，转向能不能把一段流程拆清、跑顺、反复复用并稳定交付”，而不是泛泛讨论模型参数。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮在线抓取检查返回 `API Error 400`，且没有取得可验证的深层详情；下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
2. **`openclaw` / `openclaw skill` 邻近趋势线索**
   - 可沿用此前稳定留痕的公开线索：`百度电商Skill登陆OpenClaw，五大能力全开放`（voice.newrank.cn，2026-02-13）。
...
3. **`AI skill` / `workflow skill` 邻近趋势线索**
   - `Krea AI推出节点式工作流工具Nodes，总榜排名上升34位；通义千问总榜排名攀升68位 | AI产品周榜`（newrank.cn 可见结果）可继续作为“节点式工作流能形成传播点”的邻近证据。
   - 可继续沿用稳定判断：工作流、节点式组织、技能化调用、完整链路执行能力，比单点功能更适合被内容化成“可带走路径”。
   - 本轮没有新增 `workflow skill` 或 `AI skill` 在 `search/trend/skill` 下的直连详情，因此 Topic 必须明确这是邻近趋势，不是精确详情页结论。
4. **`Copilot skill` 邻近趋势线索**
   - 当前仍只拿到 Newrank 泛站内页与榜单页级别留痕，没有明确 `Copilot skill` 直连结果。
...
- Hook：先抛出“很多人还在比提示词，但用户已经开始只收藏能直接复用的流程”这种认知反差。
- 结构：先指出旧认知失效，再展示工作流型内容为什么更容易被收藏，最后落到一条可照着走的执行路径。
- 冲突点：会提问不再足够，真正的差距转移到能否把选题、写稿、审稿、归档串成稳定闭环。
- 承诺：读者看完不是只得到一个观点，而是得到一套可模仿、可复查、可继续优化的内容生产方式。
- 证据：只能引用 Newrank 优先入口受限状态与已留痕的邻近公开线索，不能伪造详情页正文。
- CTA：引导读者对照自己的内容流程，判断自己是在生产“一次聊爽”的内容，还是在沉淀“可重复跑通”的内容资产。

### Rewrite Direction
- 保留：爆款内容里“反常识开头 + 路径拆解 + 明确收益 + 可执行结尾”的结构模式。
- 改写：所有关键表达必须换成围绕“可复用交付”“流程稳定跑完”“archive-ready 内容包”的当前主题表达，不复制任何原句。
- 强化：把“workflow skill / content pipeline / archive package”写成读者可感知的实际收益，而不是抽象概念。
- 禁止：直接复用他人爆款标题句式、原文转折句、情绪化金句；也禁止把未核实的 Newrank 详情写成事实。
...
- 正文建议重点写清三段：为什么只把 AI 当聊天框会越来越难形成可复用价值、为什么 workflow skill 更容易沉淀成可反复调用的结果、怎样把选题 / 写稿 / 审稿 / 归档串成一条可重复执行的内容生产线。
...
- 主标题建议突出：`AI技能内容开始拼可复用交付`。
- 副标题建议突出：`真正拉开差距的，是整套流程能不能稳定跑完`。
- 视觉重点放在“从单次提问到整套执行链路”的升级感，以及选题、写稿、审稿、归档能被串起来的结果感。
- 至少产出 3 组封面候选，标题与副标题尽量短促有力，页面逻辑清晰，方便后续预览与手动发布。
- 归档目录、时间戳、中文标题必须与本轮统一 package 完全一致。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-052157`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-052157+AI技能内容开始拼可复用交付，真正能拉开差距的不是多会提问，而是能不能把整套流程稳定跑完`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-052157.md`
...
- 本轮在线检查结果：尝试抓取时返回 `API Error 400`，因此本轮不能写成已成功读取详情页正文。
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260314-052157`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-052157.md`
- 发布动作：未执行
