...
4. 已复核历史 Boss 留痕与 memory 约束，确认本轮仍需保持 `date / timestamp+中文标题 / material files` 结构，并继续诚实记录来源限制。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 已确认当前没有新的可验证 Newrank 深层详情输入；因此本轮仍必须把 `https://www.newrank.cn/search/trend/skill` 写成优先入口与检查对象，并沿用已留痕的稳定 Newrank 邻近线索，明确写出详情缺失与 fallback 不可用状态。
7. 已复核近邻 Boss 样例 `Boss/BOSS-CYCLE-20260312-224939.md`，沿用其已确认的来源边界、archive package 约束、viral-copy breakdown 要求与非公开发布边界。
...
9. 已确认本轮 archive package 必须统一为：`2026-03-14 / 20260314-175850+AI技能内容真正开始拉开差距，不是谁更会写提示词，而是谁能把整条流程稳定交付出来 / material files`。
10. 已确认本轮继续只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容真正开始拉开差距，不是谁更会写提示词，而是谁能把整条流程稳定交付出来`。
- 统一归档日期：`2026-03-14`。
- 统一归档包：`2026-03-14 / 20260314-175850+AI技能内容真正开始拉开差距，不是谁更会写提示词，而是谁能把整条流程稳定交付出来 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容真正开始拉开差距，不是谁更会写提示词，而是谁能把整条流程稳定交付出来”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260314-175850` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260314-175850`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260314-175850+AI技能内容真正开始拉开差距，不是谁更会写提示词，而是谁能把整条流程稳定交付出来`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260314-175850.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260314-175850`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260314-175850.md`
- 发布动作：未执行
