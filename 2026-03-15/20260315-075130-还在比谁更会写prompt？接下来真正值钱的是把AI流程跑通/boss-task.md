...
3. 已查验当前会话可用 skill 列表，仅有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
4. 已复核现有留痕 `.role-sessions/BOSS.md`、`.role-sessions/Topic/TOPIC.md` 与最近 Boss 周期文件，沿用其已确认的来源边界、archive package 约束、viral-copy breakdown 要求与非公开发布边界。
5. 已再次尝试检查 `https://www.newrank.cn/search/trend/skill`；当前 `WebFetch` 返回 `API Error 400`（请求模型不受支持），因此本轮仍未取得可验证的 Newrank 深层详情，只能如实记录“优先入口已检查，但详情未成功获取”。
6. 已确认本轮 archive package 必须统一为：`2026-03-15 / 20260315-075130+AI技能别再只换模型先把工作流接起来 / material files`。
7. 已确认 Draft 必须重写爆款模式而不是复制句子，且 Topic / Draft 都必须保留 `viral-copy breakdown`。
8. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能别再只换模型先把工作流接起来`。
- 统一归档日期：`2026-03-15`。
- 统一归档包：`2026-03-15 / 20260315-075130+AI技能别再只换模型先把工作流接起来 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能别再只换模型先把工作流接起来”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-15`、`20260315-075130` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 内容差距不只是模型强弱，而是能不能把 workflow 接起来、拆清角色、稳定复用并交付出去”，而不是泛泛讨论模型参数。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 本轮已尝试在线检查，但 `WebFetch` 返回 `API Error 400`（请求模型不受支持），因此当前没有拿到可验证的深层详情。
   - 下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
2. **`openclaw` / `openclaw skill` 邻近趋势线索**
   - 当前本轮没有新增可验证的在线详情抓取结果；可沿用 Topic 已留痕的稳定邻近线索，作为 `OpenClaw + Skill` 仍具传播语境的方向性证据。
...
- 正文建议重点写清五段：先做认知反转、再给痛点代入、再拆 3 个流程断点、再给轻量解法、最后给收藏/评论 CTA。
- 3 个流程断点优先写：输入混乱、过程不可复用、结果没人校验。
- 轻量解法优先写：固定输入模板 + 角色拆分 + 输出复核。
...
- 避免元素：榜单截图、虚构热度、夸张收益承诺、工具测评风。
- 标题与副标题尽量短促有力，页面逻辑清晰，方便后续预览与手动发布。
- 归档目录、时间戳、中文标题必须与本轮统一 package 完全一致。

### Review 要求
- 必须核验 Topic 是否优先使用了 Newrank 作为趋势来源，或明确说明了当前受限原因。
- 必须核验 Topic 是否写清：`openclaw` 与 `openclaw skill` 当前只能沿用稳定邻近线索，`search/trend/skill` 直连详情仍缺失；`AI skill`、`workflow skill` 当前主要靠邻近趋势证据支撑；`Copilot skill` 本轮检索证据不足。
...
- 必须核验 Draft 是否真正覆盖“谁负责、怎么接、如何复用、如何校验”。
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260315-075130`。
- 必须核验最终产物仅用于预览、复审与手动发布，不自动公开发布。

## 推荐选题边界
- 优先方向：AI 技能趋势、工作流技能、OpenClaw 相关大众化关注、Copilot/Agent 协作技能、节点式工作流、内容归档、内容复盘、material files 组织。
- 内容重点：不是泛泛讲 AI，而是讲“为什么很多 AI 使用还停在换模型层，而真正开始拉开差距的是能不能把一条流程接起来、拆开、跑通、复用并稳定交付出来”。
...
- `date` 层：`2026-03-15`。
- `timestamp+中文标题` 层：`20260315-075130+AI技能别再只换模型先把工作流接起来`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260315-075130.md`
...
- 本轮在线检查结果：`WebFetch` 返回 `API Error 400`（请求模型不受支持），未取得可验证详情。
- 当前会话可用 skill 列表未提供 `yhslgg-arch/url-reader`，因此 fallback 要求只能记录为未满足的环境限制。
- 若下游需要补充趋势论证，只能沿用 Topic 已留痕的公开可见邻近结果，且仅能作为标题 / 摘要片段 / 邻近趋势方向级证据使用，不能扩写成未验证正文事实。
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260315-075130`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
- 当前内容机会点明确：用户对 AI 内容的价值判断，正从“只会换模型”转向“能不能把 workflow 接起来并稳定交付”。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260315-075130.md`
- 发布动作：未执行
