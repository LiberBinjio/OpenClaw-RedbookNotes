...
6. 已尝试获取 `https://www.newrank.cn/search/trend/skill` 页面详情，但当前抓取工具返回模型不支持错误，未取得可验证的新详情，因此只能继续沿用已留痕的稳定 Newrank 邻近线索，并明确记录详情缺失与 fallback 不可用状态。
7. 已确认本轮 archive package 必须统一为：`2026-03-15 / 20260315-031924+AI技能开始从提示词内卷，转向可复用工作流资产 / material files`。
8. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能开始从提示词内卷，转向可复用工作流资产`。
- 统一归档日期：`2026-03-15`。
- 统一归档包：`2026-03-15 / 20260315-031924+AI技能开始从提示词内卷，转向可复用工作流资产 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能开始从提示词内卷，转向可复用工作流资产”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-15`、`20260315-031924` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会写提示词，转向能不能把一段流程拆清、跑顺、反复复用”，而不是泛泛讨论模型参数。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮抓取未成功取得可验证详情；因此下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
...
3. **`AI skill` / `workflow skill` 邻近趋势线索**
   - `Krea AI推出节点式工作流工具Nodes，总榜排名上升34位；通义千问总榜排名攀升68位 | AI产品周榜`（newrank.cn 公开结果）仍可作为“节点式工作流能形成传播点”的邻近证据。
...
6. **来源限制说明**
   ...
   - 本轮对 Newrank 页面抓取还出现了工具侧模型不支持错误，因此不得新增未经验证的在线细节，只能基于已留痕标题/摘要级线索与当前稳定判断继续收口。
...
- 主标题建议突出：`AI技能开始从提示词内卷`。
- 副标题建议突出：`真正拉开差距的，是可复用工作流资产`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260315-031924`。
...
- `date` 层：`2026-03-15`。
- `timestamp+中文标题` 层：`20260315-031924+AI技能开始从提示词内卷，转向可复用工作流资产`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260315-031924.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260315-031924`，避免 Topic / Draft / Cover / Review 沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260315-031924.md`
- 发布动作：未执行
