...
4. 已复核上一轮 Boss 任务文件 `Boss/BOSS-CYCLE-20260312-024809.md`，继续沿用“单主题收口 + archive package 统一 + 来源限制如实留痕”的推进方式。
...
6. 已再次确认：如果 Newrank detail 不可用，应使用 `yhslgg-arch/url-reader` 作为 fallback；但当前会话可用 skill 列表只有 `simplify` 与 `claude-api`，没有该 skill，因此只能如实记录 fallback 要求存在但当前环境不可调用，不得伪造抓取结果。
7. 已确认 Draft 必须改写爆款模式，不能复制原句；同时 Topic 必须提供 `Viral Copy Breakdown`，Draft 也必须附带简短 `viral-copy breakdown`。
8. 已确认本轮继续只准备预览、复审与手动发布内容，不执行任何公开发布动作。
9. 已确认本轮输出必须支持 archive folder layout：`2026-03-12 / 20260312-031648+中文标题 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI工作流内容为什么开始压过单条提示词：用户真正愿意收藏的，是一套能直接复用的完整路径`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-031648+AI工作流内容为什么开始压过单条提示词：用户真正愿意收藏的，是一套能直接复用的完整路径 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI 工作流内容为什么开始压过单条提示词，因为用户真正愿意收藏的是可直接复用完整路径”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-031648` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮 Boss 未新增可验证的深层详情，因此下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
2. **workflow 叙事仍可作为邻近趋势方向**
   - 可继续沿用上一轮已记录的公开可见 Newrank 站内结果标题：`Krea AI推出节点式工作流工具Nodes，总榜排名上升34位；通义千问总榜排名攀升68位 | AI产品周榜`。
   - 可借用结论：AI 叙事正在从单功能演示转向流程化表达，`workflow` 已经能成为标题抓手。
3. **AI 办公 / copilot 邻近线索仍可用**
   - 可继续沿用上一轮已记录的公开可见 Newrank 站内结果标题：`钉钉推出下一代AI办公应用形态：钉钉ONE`。
   - 可借用结论：用户关注点正在从“某个功能点”转向“下一代 AI 办公形态 / 协作形态”。
4. **AI 创作 workflow 邻近线索仍可用**
   - 可继续沿用上一轮已记录的公开可见 Newrank 站内结果标题：`可灵2.6模型上线！语音、音效与画面一键直出 重构AI视频创作工作流`。
   - 可借用结论：内容平台更容易放大“重构工作流”这类叙事，而不是孤立的模型参数升级。
5. **历史留痕可沿用线索**
   - 可继续沿用已记录的标题/摘要级线索：`2025抖音科技内容生态报告：2025年AI兴趣用户增长翻倍`、`OpenAI推出面向科学家的免费AI原生工作台 Prism`、`微软宣布加强Copilot Think Deeper深度思考功能`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260312-031648`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-031648+AI工作流内容为什么开始压过单条提示词：用户真正愿意收藏的，是一套能直接复用的完整路径`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260312-031648.md`
...
- 当前次级风险是把“AI工作流内容为什么开始压过单条提示词”写成空洞结论，因此下游必须把“用户为什么真正愿意收藏完整路径”讲具体。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-031648.md`
- 发布动作：未执行
