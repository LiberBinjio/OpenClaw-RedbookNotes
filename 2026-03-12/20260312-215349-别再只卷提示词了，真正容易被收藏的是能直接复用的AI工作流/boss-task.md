...
4. 已读取项目 memory，确认 Boss 持续要求：只写一份 Boss 任务文件、保持 archive-ready 输出、且仅支持预览 / 复审 / 手动发布。
5. 已复核上一轮稳定 Boss 文件 `Boss/BOSS-CYCLE-20260312-121126.md`，沿用其已留痕且可验证的趋势边界与归档要求。
6. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 仍不可用，因此 fallback 要求存在，但当前会话无法调用，不能伪造抓取结果。
7. 本轮未新增可验证的 Newrank 深层详情抓取结果；因此仍只能基于 Newrank 优先入口状态与已留痕的公开标题 / 摘要级邻近线索继续收口。
8. 已确认本轮继续只准备预览、复审与手动发布内容，不执行任何公开发布动作。
9. 已确认本轮输出必须支持 archive folder layout：`2026-03-12 / 20260312-215349+AI工作流内容为什么比单条提示词更容易爆：因为用户真正想收藏的是一套能直接照着走的Skill路径 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI工作流内容为什么比单条提示词更容易爆：因为用户真正想收藏的是一套能直接照着走的Skill路径`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-215349+AI工作流内容为什么比单条提示词更容易爆：因为用户真正想收藏的是一套能直接照着走的Skill路径 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI工作流内容为什么比单条提示词更容易爆，因为用户真正想收藏的是一套能直接照着走的Skill路径”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-215349` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“用户更愿意收藏能直接复用的工作流内容”，而不是泛泛讨论某个 AI 工具是否更强。
...
- 主标题建议突出：`AI工作流内容为什么比单条提示词更容易爆`。
- 副标题建议突出：`用户真正想收藏的不是一句提示词，而是一套Skill路径`。
...
- 必须核验 Topic 是否把 `date`、`timestamp`、`archive_package` 全量更新为 `2026-03-12` 与 `20260312-215349`。
- 必须核验 Draft 是否包含 `Viral Rewrite Notes` 与简短 `viral-copy breakdown`。
- 必须核验 Draft 是否保持“结构改写而非句子复制”。
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260312-215349`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-215349+AI工作流内容为什么比单条提示词更容易爆：因为用户真正想收藏的是一套能直接照着走的Skill路径`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260312-215349.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260312-215349`，避免归档包再次不一致。
...
## 结果
- 分支检查：通过（`RedbookClaw`）
- 指令文件读取：完成（`Boss/CLAUDE.md`、`.role-sessions/current_task.md`、memory）；未发现可用的 `yhslgg-arch/url-reader` skill
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-215349.md`
- 发布动作：未执行
