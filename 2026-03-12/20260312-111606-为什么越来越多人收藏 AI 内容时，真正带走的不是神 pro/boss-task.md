...
5. 已复核上一轮稳定 Boss 任务文件 `Boss/BOSS-CYCLE-20260312-104610.md`，沿用其已留痕且可验证的趋势边界与归档要求。
6. 已再次确认当前环境可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 仍不可用，因此 fallback 要求存在，但当前会话无法调用，不能伪造抓取结果。
7. 本轮未新增可验证的 Newrank 深层详情抓取结果，因此仍只能基于 Newrank 优先入口状态与已留痕的公开标题 / 摘要级邻近线索继续收口，不能把 `https://www.newrank.cn/search/trend/skill` 写成已验证正文来源。
8. 已确认本轮继续只准备预览、复审与手动发布内容，不执行任何公开发布动作。
9. 已确认本轮输出必须支持 archive folder layout：`2026-03-12 / 20260312-111606+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-111606+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-111606` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic 是否把 `date`、`timestamp`、`archive_package` 全量更新为 `2026-03-12` 与 `20260312-111606`。
- 必须核验 Draft 是否包含 `Viral Rewrite Notes` 与简短 `viral-copy breakdown`。
- 必须核验 Draft 是否保持“结构改写而非句子复制”。
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260312-111606`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-111606+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260312-111606.md`
...
- 可继续沿用的历史留痕结果见：
  - `Boss/BOSS-CYCLE-20260312-104610.md:38-64`
  - `Boss/CLAUDE.md:3-21`
...
- 当前次级风险是 archive package 漂移：上一轮统一包是 `20260312-104610`，本轮下游必须刷新为 `20260312-111606`，避免 Topic / Draft / Cover / Review 分属不同归档包。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-111606.md`
- 发布动作：未执行
