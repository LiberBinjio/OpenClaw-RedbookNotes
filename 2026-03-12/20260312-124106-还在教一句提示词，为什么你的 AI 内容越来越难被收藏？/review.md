...
## Archive Package
- date: `2026-03-12`
- timestamp: `20260312-124106`
- archive_title: `AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`
- archive_package: `2026-03-12 / 20260312-124106+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径 / material files`
...
- Cover 已切到本轮要求的 `20260312-124106`，但 Topic 与 Draft 仍停留在 `20260312-121126`，因此当前 archive package 尚未完全统一，见 `.role-sessions/Topic/@TOPIC:3-8`、`Draft/@DRAFT.md:3-8`、`Cover/@COVER:3-8`。
...
CONDITIONAL
- 中文物料本身已具备：Draft 提供中文标题、中文正文、标签与 settings，Cover 提供中文封面方案，满足 archive-ready 中文物料的内容项要求，见 `Draft/@DRAFT.md:10-76`、`Cover/@COVER:19-79`。
- 但 archive timestamp 仍不一致：Topic 与 Draft 还是 `20260312-121126`，Cover 与本 Review 为 `20260312-124106`，不满足“同一归档包完全统一”的硬性要求，见 `.role-sessions/Topic/@TOPIC:3-8`、`Draft/@DRAFT.md:3-8`、`Cover/@COVER:3-8`。
...
1. 先把 Topic 与 Draft 的 `timestamp` 和 `archive_package` 从 `20260312-121126` 全量更新到 `20260312-124106`，确保 Topic / Draft / Cover / Review 指向同一归档包。
2. 若要从 `CONDITIONAL` 升级到 `PASS`，还必须提供可用的 `yhslgg-arch/url-reader`，或给出等价且真实的 fallback 抓取记录，用于补齐 Newrank detail 缺失时的 required fallback 证据。
3. fallback 补齐后，Topic 需要追加记录：实际抓取了什么 URL、抓到了哪些可核实字段、哪些内容仍不可确认。
4. 在上述两项未补齐前，当前包可用于预览、复审和手动发布准备，但不能宣称来源证据链与归档包一致性已经完全闭环。
