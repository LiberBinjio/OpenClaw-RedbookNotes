...
6. 已确认本轮 Topic 仍必须优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写进来源状态；但当前没有新增可验证的 Newrank 深层详情抓取结果，因此仍只能沿用已留痕的稳定 Newrank 邻近线索，并明确记录详情缺失与 fallback 不可用状态。
...
8. 已确认本轮 archive package 必须统一为：`2026-03-12 / 20260312-212624+AI技能内容真正开始分层，最后被反复带走的不是一句神提示词，而是一整套能复用的工作流 / material files`。
9. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容真正开始分层，最后被反复带走的不是一句神提示词，而是一整套能复用的工作流`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-212624+AI技能内容真正开始分层，最后被反复带走的不是一句神提示词，而是一整套能复用的工作流 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容真正开始分层，最后被反复带走的不是一句神提示词，而是一整套能复用的工作流”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-212624` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会写提示词，转向能不能把一段流程拆清、跑顺、反复复用”，而不是泛泛讨论模型参数或单次回答。
...
- 副标题建议突出：`最后被带走的，是可复用工作流`。
- 视觉重点放在“从单次提示词到整套执行链路”的升级感，以及选题、写稿、审稿、归档能被串起来的结果感。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260312-212624`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-212624+AI技能内容真正开始分层，最后被反复带走的不是一句神提示词，而是一整套能复用的工作流`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260312-212624.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260312-212624`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
- 当前内容机会点明确：用户对 AI 内容的价值判断，正从“会不会写 prompt”转向“能不能把一段流程稳定拆开、跑通并复用”。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-212624.md`
- 发布动作：未执行
