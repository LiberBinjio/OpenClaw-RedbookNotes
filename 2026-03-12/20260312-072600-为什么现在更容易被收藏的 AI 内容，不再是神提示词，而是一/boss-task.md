...
5. 已检查持久记忆入口 `memory/MEMORY.md`，当前不存在可复用内容，因此不引入未验证历史结论。
6. 已复核上一轮 Boss 文件 `Boss/BOSS-CYCLE-20260312-065751.md` 与近邻轮次文件，确认继续沿用“单主题收口 + archive package 统一 + 来源限制如实留痕”的推进方式。
...
8. 已再次确认：如果 Newrank detail 不可用，应使用 `yhslgg-arch/url-reader` 作为 fallback；但当前会话可用 skill 只有 `simplify` 与 `claude-api`，没有该 skill，因此只能如实记录 fallback 要求存在但当前环境不可调用，不得伪造抓取结果。
9. 本轮补充核实到 3 条可留痕的 Newrank 邻近趋势线索：
   - `Kimi推出OpenClaw云托管服务，跻身总榜前十；字节Seedance 2.0助力即梦排名上升 | AI产品月榜`（2026-03-06，newrank.cn / voice.newrank.cn）
   - `1条视频涨粉40万，科普圈“掌管AI的神”首次公开幕后制作全流程 | AI新榜独家`（newrank.cn / voice.newrank.cn）
   - `字节Trae上线Skills功能，总榜排名上升30位；PixVerse R1模型发布，海外榜排名上升21位 | AI产品周榜`（2026-01-22，newrank.cn）
10. 本轮未直接拿到 `https://www.newrank.cn/search/trend/skill` 的详情页内容；当前仅能把该 URL 视为优先入口，不得扩写为已验证细节页事实。
11. 已确认 Draft 必须改写爆款模式，不能复制原句；同时 Topic 与 Draft 必须保留 `viral-copy breakdown` / `Viral Copy Breakdown`。
12. 已确认本轮继续只准备预览、复审与手动发布内容，不执行任何公开发布动作。
13. 已确认本轮输出必须支持 archive folder layout：`2026-03-12 / 20260312-072600+中文标题 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容为什么越来越像“工作流说明书”：真正让用户愿意收藏的，不是零散提示词，而是一套能直接照着复用的路径`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-072600+AI技能内容为什么越来越像“工作流说明书”：真正让用户愿意收藏的，不是零散提示词，而是一套能直接照着复用的路径 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容为什么越来越像工作流说明书，因为用户真正愿意收藏的不是零散提示词，而是一套能直接照着复用的路径”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-072600` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“用户更愿意保存能直接复用的工作流内容”，而不是泛泛讨论某个 AI 工具是否更强。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前已确认该入口是任务要求中的优先来源，但本轮没有拿到可验证详情页正文，因此下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
2. **`openclaw` 邻近趋势线索**
   - 已补充核实到 Newrank 文章：`Kimi推出OpenClaw云托管服务，跻身总榜前十；字节Seedance 2.0助力即梦排名上升 | AI产品月榜`（2026-03-06）。
   - 该线索至少说明：`openclaw` 已进入新榜 AI 内容语境中的显性关键词，可支持 Topic 写“openclaw 相关关注度已进入更大众化内容语境”，但不能替代 `search/trend/skill` 详情页本身。
3. **`workflow` / 全流程公开邻近趋势线索**
   - 已补充核实到 Newrank 文章：`1条视频涨粉40万，科普圈“掌管AI的神”首次公开幕后制作全流程 | AI新榜独家`。
   - 该线索可支撑结论：AI 相关内容的传播抓手不只在工具名，而在“完整流程公开”“可复用工作流展示”“幕后全流程拆解”这类路径型表达。
4. **`Skills` 化表达信号**
   - 已补充核实到 Newrank 周榜线索：`字节Trae上线Skills功能，总榜排名上升30位；PixVerse R1模型发布，海外榜排名上升21位 | AI产品周榜`（2026-01-22）。
...
5. **关键词命中状态说明**
   ...
   - `workflow skill`：没有直连结果，但拿到了 `全流程公开` 与 `Skills` 邻近线索。
...
6. **来源限制说明**
   ...
   - 本轮不得新增未经验证的在线细节，只能基于已核实标题/摘要级线索继续收口。
...
- 必须核验 Topic 是否写清：`openclaw` 有稳定 Newrank 邻近线索，但 `openclaw skill`、`AI skill`、`Copilot skill` 仍缺稳定直连结果；`workflow` / `全流程公开` / `Skills` 有可用邻近趋势证据。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260312-072600`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-072600+AI技能内容为什么越来越像“工作流说明书”：真正让用户愿意收藏的，不是零散提示词，而是一套能直接照着复用的路径`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260312-072600.md`
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-072600.md`
- 发布动作：未执行
