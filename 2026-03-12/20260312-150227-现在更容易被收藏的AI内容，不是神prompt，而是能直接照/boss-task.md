...
2. 已读取 `.role-sessions/current_task.md`，确认 Topic 仍必须优先从 `https://www.newrank.cn/search/trend/skill` 获取趋势线索，并产出 viral-copy breakdown。
3. 已按要求执行 `git branch --show-current`，当前分支为 `RedbookClaw`，符合继续工作的前提。
4. 已尝试定位 `SAF-TaskAssignAndProgress.md`；当前仓库中不存在该文件，已如实记录缺失状态。
5. 已复核近邻稳定 Boss 任务文件 `Boss/BOSS-CYCLE-20260312-143332.md` 与 `Boss/BOSS-CYCLE-20260312-140436.md`，沿用其已留痕且可验证的趋势边界、archive package 约束与非公开发布边界。
6. 已再次确认当前环境可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法调用，不能伪造抓取结果。
7. 已再次补做 Newrank 邻近公开搜索，新增/确认可见线索包括：`百度电商Skill登陆OpenClaw，五大能力全开放`、`Kimi推出OpenClaw云托管服务，跻身总榜前十；字节Seedance 2.0助力即梦排名上升 | AI产品月榜`、`百度智能云推出移动端OpenClaw部署方案`、`百度App接入OpenClaw 2周，超2成用户调用于投资理财分析`、`Kimi跃居国内榜前三…`、`Minimax推出MaxClaw`。
8. 已再次确认 `workflow skill` 与 `Copilot skill` 在当前可见检索里仍未拿到 `search/trend/skill` 直连详情；`workflow skill` 仍主要依赖 `Krea AI推出节点式工作流工具Nodes... | AI产品周榜` 等邻近证据，`Copilot skill` 当前仅检到 Newrank 泛站内页与 AI 榜单页，没有明确直连结果。
9. 已确认本轮继续只准备预览、复审与手动发布内容，不执行任何公开发布动作。
10. 已确认本轮输出必须支持 archive folder layout：`2026-03-12 / 20260312-150227+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-150227+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-150227` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
4. **`Copilot skill` 邻近趋势线索**
   - 本轮补搜 `site:newrank.cn Copilot Newrank 2026 skill`，仅返回 Newrank 首页、话题榜与 AI 账号榜等泛站内页，没有明确 `Copilot skill` 直连结果。
   - 因此 `Copilot skill` 本轮只能写成“检索不足、暂无直连详情”，不能伪造成已抓到趋势正文。
5. **关键词命中状态说明**
   - `openclaw`：本轮有稳定 Newrank 邻近线索，且新增 OpenClaw 云托管 / 移动端部署 / App 接入等可见结果。
   ...
   - `Copilot skill`：本轮未获得明确直连结果，仅有 Newrank 泛站内页，证据最弱。
6. **来源限制说明**
   ...
   - 本轮不得新增未经验证的在线细节，只能基于已留痕标题/摘要级线索与本轮可见搜索结果继续收口。
...
- 必须核验 Topic 是否把 `date`、`timestamp`、`archive_package` 全量更新为 `2026-03-12` 与 `20260312-150227`。
- 必须核验 Draft 是否包含 `Viral Rewrite Notes` 与简短 `viral-copy breakdown`。
- 必须核验 Draft 是否保持“结构改写而非句子复制”。
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260312-150227`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-150227+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260312-150227.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260312-150227`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-150227.md`
- 发布动作：未执行
