...
4. 已检查上一轮 Boss 文件 `Boss/BOSS-CYCLE-20260312-012309.md`、延续参考 `Boss/BOSS-CYCLE-20260312-005644.md` 与趋势来源参考 `Boss/BOSS-CYCLE-20260311-233400.md`，继续沿用“单主题收口 + archive package 统一 + 来源限制如实留痕”的方式推进。
...
6. 已确认当前会话明确要求：如果 Newrank 详情不可用，要使用 `yhslgg-arch/url-reader` 作为 fallback；但当前可用 skill 列表里只有 `simplify` 与 `claude-api`，没有该 skill，因此本轮只能如实记录 fallback 要求存在但当前环境不可调用，不得伪造抓取结果。
7. 已确认 Draft 必须改写爆款模式，不能复制原句；同时 Topic / Draft 仍需保留 viral-copy breakdown。
8. 已确认本轮继续只准备预览与手动发布内容，不执行任何公开发布动作。
9. 已确认本轮统一归档日期为 `2026-03-12`，并以 `20260312-015149+中文标题` 作为 archive package 的 timestamp 层，以支持 `date / timestamp+中文标题 / material files` 布局。
10. 已确认仓库已存在 `2026-03-12/` 归档日期层，因此本轮下游应直接落入同一天的新 timestamp 包，而不是另起日期层。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260312-015149+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260312-015149` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover 的 archive timestamp 与 archive package 是否统一到 `20260312-015149`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260312-015149+AI工作流内容为什么更容易被收藏和复用：因为用户真正想带走的不是一句提示词，而是一整套可直接照着走的路径`。
...
- 当前可沿用的稳妥线索主要来自：
  - `Boss/BOSS-CYCLE-20260311-233400.md:39-58`
  - `Boss/BOSS-CYCLE-20260312-005644.md:38-57`
  - `Boss/BOSS-CYCLE-20260312-012309.md:39-58`
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260312-015149.md`
- 发布动作：未执行
