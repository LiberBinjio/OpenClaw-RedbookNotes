...
4. 已复核既有留痕 `Boss/BOSS-CYCLE-20260318-085310.md`、`Boss/BOSS-CYCLE-20260318-092006.md`、根目录 `@TOPIC`、`Draft/@DRAFT.md`、`Cover/@COVER`、`Review/@REVIEW`，确认当前主命题仍应围绕“AI 技能竞争点已从提示词技巧转向流程交付能力”继续收口。
5. 已按要求优先检查 `https://www.newrank.cn/search/trend/skill` 的抓取可用性；当前 `WebFetch` 返回 `API Error 400`，因此本轮不能把 Newrank 检查写成成功取数。
6. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 required fallback 存在，但本会话无法执行，不能伪造抓取结果。
7. 已确认当前跨文件 archive package 仍不一致：`Draft/@DRAFT.md` 与 `Review/@REVIEW` 维持 `20260318-085310`，`Cover/@COVER` 已漂移到 `20260318-094647`，根目录 `@TOPIC` 仍停在 `20260318-092006`；因此本轮 Boss 任务必须优先要求全链路重新统一归档包。
8. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。
9. 已确认本轮统一 archive package 必须切为：`2026-03-18 / 20260318-101513+AI技能内容别再只卷提示词真正拉开差距的是能不能把流程接成可交付workflow / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容别再只卷提示词真正拉开差距的是能不能把流程接成可交付workflow`。
- 统一归档日期：`2026-03-18`。
- 统一归档包：`2026-03-18 / 20260318-101513+AI技能内容别再只卷提示词真正拉开差距的是能不能把流程接成可交付workflow / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容别再只卷提示词真正拉开差距的是能不能把流程接成可交付workflow”的主选题。
...
- Topic 的 `date`、`timestamp`、`archive_title`、`archive_package` 必须统一更新到本轮：`2026-03-18`、`20260318-101513`、`AI技能内容别再只卷提示词真正拉开差距的是能不能把流程接成可交付workflow`。
- 在趋势表达上，要优先收口到“AI 的差距不只在模型，而在能不能把流程拆清、接顺、复用、复核，并最终产出可交付结果”，而不是泛泛讨论模型参数。
...
- 必须保留 `Viral Rewrite Notes` 或等价段落，清楚说明这是对爆款结构的原创改写，不是句子复制。
- 正文建议重点写清五段：先做认知反转、再讲结果忽高忽低的真实痛点、中段拆 3 个流程断点、给出轻量解法、最后收口到收藏/评论型 CTA。
- Draft 必须产出中文标题、中文正文、标签，并附带简短 `viral-copy breakdown` 区块，满足当前任务要求。
- Draft 的 `date`、`timestamp`、`archive_title`、`archive_package` 也必须同步切到本轮统一值，不得继续停留在 `20260318-085310`。
- 不得公开发布，不得写成自动发布指令。

### Cover 要求
- 封面方向必须与统一中文标题一致，并支持归档目录下独立留存。
- 主标题建议突出：`AI技能内容别再只卷提示词`。
- 副标题建议突出：`真正拉开差距的是可交付workflow`。
- 视觉重点放在“流程断点、协作链路、稳定交付”的升级感，以及输入、拆分、复核、归档被串起来后的结果感。
- 标题与副标题尽量短促有力，页面逻辑清晰，方便后续预览与手动发布。
- Cover 必须输出至少 3 个候选，并把 `date`、`timestamp`、`archive_title`、`archive_package` 统一到本轮 package，避免继续使用 `20260318-094647`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否全部统一到 `20260318-101513`。
- 必须核验最终产物仅用于预览、复审与手动发布，不自动公开发布。

## 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮通过 `WebFetch` 检查时返回 `API Error 400`，因此不能把这次检查写成成功取数，也不能把 `search/trend/skill` 扩写成已核实正文事实。
2. **`openclaw` / `openclaw skill` 邻近趋势线索**
   - 可沿用当前已留痕的 Newrank 邻近公开线索：`百度电商Skill登陆OpenClaw，五大能力全开放`、`Kimi推出OpenClaw云托管服务，跻身总榜前十；字节Seedance 2.0助力即梦排名上升 | AI产品月榜`、`百度智能云推出移动端OpenClaw部署方案`、`百度App接入OpenClaw 2周，超2成用户调用于投资理财分析`。
   - 这些线索可支持“OpenClaw + Skill / 落地工作流 / 可复用能力”正在传播语境中活跃的表达。
   - 以上线索都属于 Newrank 域名下的公开邻近文章，不是 `search/trend/skill` 深层详情页本身。
3. **`AI skill` / `workflow skill` 邻近趋势线索**
   - 可沿用已留痕的稳定判断：节点式工作流、技能化调用、完整链路执行能力，比单点功能更适合被内容化成“可带走路径”。
   - `Krea AI推出节点式工作流工具Nodes，总榜排名上升34位；通义千问总榜排名攀升68位 | AI产品周榜` 可继续作为“工作流更容易成为传播点”的邻近证据。
   - 本轮没有拿到 `workflow skill` 在 `search/trend/skill` 下的直连详情，因此 Topic 必须明确这是邻近趋势，不是精确详情页结论。
4. **`Copilot skill` 邻近趋势线索**
   - 当前仍未拿到明确 `Copilot skill` 直连结果。
...
5. **关键词命中状态说明**
   ...
   - `Copilot skill`：本轮未获得明确直连结果，仅可记录为证据不足。
6. **来源限制说明**
...
- 内容重点：不是泛泛讲 AI，而是讲“为什么很多 AI 使用者以为问题出在模型，实际上更常见的问题是输入没统一、流程没拆开、结果没人复核，所以无法形成可交付结果”。
...
- `date` 层：`2026-03-18`。
- `timestamp+中文标题` 层：`20260318-101513+AI技能内容别再只卷提示词真正拉开差距的是能不能把流程接成可交付workflow`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260318-101513.md`
  - `@TOPIC`
  - `Draft/@DRAFT.md`
  - `Cover/@COVER`
  - `Review/@REVIEW`
...
- 本轮实际在线检查状态：`WebFetch` 返回 `API Error 400`，因此不能把本轮记为成功抓取详情。
...
- 当前次级风险是 archive package 漂移：截至本轮检查，`@TOPIC`、`@DRAFT.md`、`@COVER`、`@REVIEW` 分别停留在不同时间戳，所以下游必须先统一归档再继续预览与复审。
- 当前内容机会点明确：用户对 AI skill 的判断，正在从“模型会不会更会聊”转向“流程能不能更稳定、更能复用、更方便交付”。
...
- 来源状态：已优先检查 Newrank 入口，但在线抓取失败；required fallback skill 当前不可用
- 实际改动：新增 `Boss/BOSS-CYCLE-20260318-101513.md`
- 发布动作：未执行
