...
4. 已确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
5. 已再次确认本轮 Topic 仍必须优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写进来源状态；但当前会话没有可验证的新抓取详情，因此只能沿用已留痕的稳定 Newrank 邻近线索，并明确写出详情缺失与 fallback 不可执行状态。
6. 已确认本轮仍应优先围绕 `openclaw`、`openclaw skill`、`AI skill`、`workflow skill`、`Copilot skill` 组织 Topic 检索与表达，并如实区分直连详情、邻近信号与证据不足三种状态。
7. 已确认本轮 archive package 必须统一为：`2026-03-18 / 20260318-104323+AI技能真正开始拉开差距，不在提示词多高级，而在你能不能把一条流程稳定跑完 / material files`。
...
- 统一归档日期：`2026-03-18`。
- 统一归档包：`2026-03-18 / 20260318-104323+AI技能真正开始拉开差距，不在提示词多高级，而在你能不能把一条流程稳定跑完 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-18`、`20260318-104323` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的价值正在从会不会写提示词，升级成能不能稳定跑通并交付一整条流程”，而不是泛泛讨论模型能力。
- 必须产出中文 `viral-copy breakdown`，拆清楚爆款结构为什么有效，以及本轮如何做非抄袭重写。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮没有拿到可验证的深层详情；因此下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
...
- 副标题建议突出：`拼的不是提示词，而是流程稳定跑完`。
- 视觉重点放在“从一次性回答到整条流程跑通”的升级感，以及选题、写稿、审稿、归档能被串起来的结果感。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260318-104323`。
- 必须核验最终产物仅用于预览、复审与手动发布，不自动公开发布。

## 推荐选题边界
- 优先方向：AI 技能趋势、工作流技能、OpenClaw 相关大众化关注、Copilot/Agent 协作技能、节点式工作流、内容归档、内容复盘、material files 组织。
- 内容重点：不是泛泛讲 AI，而是讲“为什么很多 AI 内容还停在提示词层，而真正开始拉开差距的是能不能把一条流程稳定跑完”。
...
- `date` 层：`2026-03-18`。
- `timestamp+中文标题` 层：`20260318-104323+AI技能真正开始拉开差距，不在提示词多高级，而在你能不能把一条流程稳定跑完`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260318-104323.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260318-104323`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
- 当前内容机会点明确：用户对 AI 内容的价值判断，正从“会不会写 prompt”转向“能不能把一条流程稳定跑通并交付出来”。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260318-104323.md`
- 发布动作：未执行
