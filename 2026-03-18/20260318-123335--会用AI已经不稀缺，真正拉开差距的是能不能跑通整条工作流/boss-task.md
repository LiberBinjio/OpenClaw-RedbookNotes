...
4. 已复核最近稳定留痕 `Boss/BOSS-CYCLE-20260318-120439.md`、`.role-sessions/BOSS.md` 与 `.role-sessions/Topic/TOPIC.md`，沿用其已确认的来源边界、archive package 约束、viral-copy breakdown 要求与非公开发布边界。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. Newrank 仍是本轮一手优先入口：`https://www.newrank.cn/search/trend/skill`。当前会话未取得可核验详情，因此本轮只能继续将其写成优先检查对象与来源边界，不能扩写为已验证细节。
7. 已确认本轮 Topic 仍应优先围绕 `openclaw`、`openclaw skill`、`AI skill`、`workflow skill`、`Copilot skill` 组织检索与表达，并如实区分直连详情、邻近信号与证据不足三种状态。
8. 已确认本轮 archive package 必须统一为：`2026-03-18 / 20260318-123335+AI技能内容开始拼交付闭环，不再拼谁会多写一句提示词 / material files`。
9. 已确认本轮继续只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始拼交付闭环，不再拼谁会多写一句提示词`。
- 统一归档日期：`2026-03-18`。
- 统一归档包：`2026-03-18 / 20260318-123335+AI技能内容开始拼交付闭环，不再拼谁会多写一句提示词 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始拼交付闭环，不再拼谁会多写一句提示词”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-18`、`20260318-123335` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的价值正在从会不会写提示词，升级成能不能稳定跑通并交付一整条流程”，而不是泛泛讨论模型能力。
- 必须产出 viral-copy breakdowns，拆清 hook、结构、CTA、节奏与情绪触发点，但只能做结构改写，不能复制原句。

### 本轮可用趋势线索
1. **Newrank 优先入口状态**
   - 一手入口仍是 `https://www.newrank.cn/search/trend/skill`。
   - 当前任务要求是优先使用该入口，但本轮仍未拿到可验证的深层详情；当前会话也没有可调用的 `yhslgg-arch/url-reader` skill，因此下游只能把它写成优先入口与检查对象，不能扩写成已核实正文事实。
2. **`openclaw` / `openclaw skill` 邻近趋势线索**
   - 可沿用 Topic 已留痕的方向性判断：`OpenClaw + Skill` 相关叙事更适合被写进“能力开放、流程落地、可复用节点”的传播语境。
   - 这些线索当前只能作为 Newrank 域内或任务关键词约束下的邻近信号使用，不是 `search/trend/skill` 深层详情页本身。
3. **`AI skill` / `workflow skill` 邻近趋势线索**
   - 当前可安全使用的方向是：用户注意力正在从单点 prompt 技巧，转向工作流、角色分工、复用模板、复核闭环等完整路径。
   - 这些线索只可支撑“从单次提问转向流程化协作”的方向判断，不能伪装成 `search/trend/skill` 精确详情页结论。
4. **`Copilot skill` 邻近趋势线索**
   - 当前仍未获得明确 `Copilot skill` 直连详情，只能写成“检索不足、暂无直连结果”的弱证据状态。
   - 因此 `Copilot skill` 本轮不能被写成已抓到趋势正文。
5. **关键词命中状态说明**
   - `openclaw`：可作为稳定邻近线索方向使用。
   - `openclaw skill`：可作为 `OpenClaw + Skill` 邻近文章方向使用，但仍非 `search/trend/skill` 直连详情。
   - `AI skill`：没有直连详情，但方向上可支撑“AI 内容从问答走向流程化交付”。
   - `workflow skill`：没有直连详情，但可支撑“节点式工作流、角色协作、闭环复核”等方向。
   - `Copilot skill`：检索证据最弱，只能诚实写不足。
6. **来源限制说明**
   - 当前能确认的是：Newrank 优先入口存在，但本轮未拿到可核验详情。
   - 指定 fallback `yhslgg-arch/url-reader` 在当前会话不可用，因此不能把 fallback 写成已执行成功。
   - 本轮不得新增未经验证的在线细节，只能基于任务指定关键词方向与当前稳定可验证限制继续收口。
...
- 必须单独附上一个简短 `viral-copy breakdown` 区块，清楚说明 hook、结构、CTA、节奏与情绪触发点是如何重写的。
...
- 主标题建议突出：`AI技能内容开始拼交付闭环`。
- 副标题建议突出：`拼的不是提示词，是整条流程能不能落地`。
...
- 必须核验 Topic 是否写清：`openclaw` 与 `openclaw skill` 当前仅有邻近线索；`AI skill`、`workflow skill` 当前主要靠方向性趋势支撑；`Copilot skill` 本轮检索证据不足。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260318-123335`。
...
- `date` 层：`2026-03-18`。
- `timestamp+中文标题` 层：`20260318-123335+AI技能内容开始拼交付闭环，不再拼谁会多写一句提示词`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260318-123335.md`
...
- 本轮只能把该入口写成优先检查对象与来源边界，不能写成已成功取到详情。
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260318-123335`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260318-123335.md`
- 发布动作：未执行
