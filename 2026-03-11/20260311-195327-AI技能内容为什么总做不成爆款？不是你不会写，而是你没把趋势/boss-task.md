...
5. 已复核既有 Boss 周期文件与当前任务要求，确认本轮仍应延续“单一主题 + 单一归档包 + 单一 Boss 留痕文件”的收口方式。
...
7. 当前会话可确认 fallback 要求存在，但环境内未见可调用的 `yhslgg-arch/url-reader` skill；因此只能要求下游在执行时先尝试该 fallback，如仍不可用，必须如实记录不可用状态，不得伪造抓取详情。
...
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260311-195327+AI技能内容为什么总做不成爆款？不是你不会写，而是你没把趋势、工作流和归档串起来 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260311-195327` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover 的 archive timestamp 与 archive package 是否统一到 `20260311-195327`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260311-195327+AI技能内容为什么总做不成爆款？不是你不会写，而是你没把趋势、工作流和归档串起来`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260311-195327.md`
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260311-195327.md`
- 发布动作：未执行
