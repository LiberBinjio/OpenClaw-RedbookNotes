...
4. 已检查记忆文件 `memory/MEMORY.md`，当前无现成稳定记忆可复用，不额外引入旧结论。
5. 已复核当前任务提示，确认本轮仍应延续“单一主题 + 单一归档包 + 单一 Boss 留痕文件”的收口方式。
6. 已按要求继续将 `https://www.newrank.cn/search/trend/skill` 作为 Topic 的首要趋势入口，并要求 Topic 优先围绕 `openclaw`、`openclaw skill`、`AI skill`、`workflow skill`、`Copilot skill` 组织趋势检索。
7. 当前任务要求写明：若 Newrank 详情不可用，必须先走 `yhslgg-arch/url-reader` 作为 fallback；但当前会话可见技能列表中未出现该 skill，因此只能要求下游先检查并尝试，如仍不可用，必须如实记录不可用状态，不得伪造抓取详情。
8. 已确认本轮继续只准备预览与手动发布内容，不执行公开发布。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`为什么你做了那么多 AI 技能内容，最后还是没有一条跑出来？问题不是不会写，而是没把趋势、工作流和归档连成闭环`。
- 统一归档日期：`2026-03-12`。
- 统一归档包：`2026-03-12 / 20260311-204701+为什么你做了那么多 AI 技能内容，最后还是没有一条跑出来？问题不是不会写，而是没把趋势、工作流和归档连成闭环 / material files`。
...
- 如果 Newrank 详情不可用，必须先尝试 `yhslgg-arch/url-reader` fallback；若当前环境仍不存在或不可用，必须如实写出不可用状态，不能伪造抓取结果。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-12`、`20260311-204701` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover 的 archive timestamp 与 archive package 是否统一到 `20260311-204701`。
...
- `date` 层：`2026-03-12`。
- `timestamp+中文标题` 层：`20260311-204701+为什么你做了那么多 AI 技能内容，最后还是没有一条跑出来？问题不是不会写，而是没把趋势、工作流和归档连成闭环`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260311-204701.md`
...
## 当前判断
- 当前最大风险仍不在写作，而在来源留痕失真：Newrank 详情拿不到，就必须诚实写限制；fallback skill 若不可用，也必须诚实写不可用。
...
- 记忆文件状态：`memory/MEMORY.md` 不存在
- 实际改动：新增 `Boss/BOSS-CYCLE-20260311-204701.md`
- 发布动作：未执行
