...
归档路径建议：2026-04-06 / 20260406-030121+AI技能越多越要接流程，真正拉开差距的是稳定交付 / material files
...
- 严格遵守边界：仅在 `RedbookClaw` 工作，不触碰 `master/main`，不修改 `node5`，且口径只面向 `@ALL`。
...
4. 已复核 `.role-sessions/BOSS.md` 与 `.role-sessions/Topic/TOPIC.md`，沿用已确认的来源边界、archive-ready 约束、viral-copy breakdown 要求与非公开发布边界。
5. 首要来源仍固定为 `https://www.newrank.cn/search/trend/skill`；本轮只可写明其为优先检查入口，当前未取得可复核的 `search/trend/skill` detail。
6. 规则要求 detail 不可用时使用 `yhslgg-arch/url-reader` 作为 required fallback，但当前会话可用 skill 只有 `simplify` 与 `claude-api`，因此本轮不能把 fallback 写成已执行完成。
7. 本轮趋势主线继续收口为：`AI技能越多越要接流程，真正拉开差距的是稳定交付`。
8. 本轮所有下游产物必须统一归档到：`2026-04-06 / 20260406-030121+AI技能越多越要接流程，真正拉开差距的是稳定交付 / material files`。

## 3. 本轮统一归档包
- date: `2026-04-06`
- timestamp: `20260406-030121`
- archive_title: `AI技能越多越要接流程，真正拉开差距的是稳定交付`
- archive_package: `2026-04-06 / 20260406-030121+AI技能越多越要接流程，真正拉开差距的是稳定交付 / material files`
...
- 归档要求：所有下游产物按 `2026-04-06 / 20260406-030121+AI技能越多越要接流程，真正拉开差距的是稳定交付 / material files` 对齐命名与整理。
...
- 当前来源核验结论：
  ...
  - 当前没有拿到可复核的 `search/trend/skill` detail 页面内容。
  - 本轮不能把该入口写成已成功抓到详情页。
...
- 因此本轮可安全使用的证据边界只有两类：
  - Newrank 邻近公开标题、日期、摘要级线索。
  - `Newrank 已优先检查但 exact detail 未取得，required fallback skill 当前不可执行` 这一事实本身。
- 本轮不可写成事实的内容：任何未直接核验的排名、热度、互动量、详情页正文、截图结论、爆文原句。
- 当前可继承的趋势判断：围绕 OpenClaw、skills、多人协作、节点式 workflow 的公开讨论正在汇合，内容重点正在从“又多了什么工具/Skill”转向“这些 Skill 能不能被接成稳定交付链路”。
...
- Topic 必须产出 `Viral Copy Breakdown`、`Rewrite Direction`，并明确写出 fetched source URL 与实际 extraction path；若 fallback 不可执行，需如实写出未执行原因。

## 6. 给 Topic / Draft 的明确交付口径
...
- Topic 需要如实写出关键词命中状态：
  ...
  - `AI skill`：可由技能生态、平台化 skills、场景化接入等邻近信号支撑。
  - `workflow skill`：可由“11种经典玩法”、节点式调用、多人协作等邻近信号支撑。
...
- 证据使用方式：
  - 可以写：Newrank 已优先检查；本轮未成功取得 `search/trend/skill` 可复核 detail；required fallback `yhslgg-arch/url-reader` 当前环境不可用；趋势判断建立在 Newrank 邻近公开标题级线索上。
...
- 如上述任一项不成立，Review 应返回 `FAIL` 或 `CONDITIONAL`，并列出明确返工项。

## 9. Viral Copy Breakdown 要求
...
- Proof：证据只允许落到 Newrank 公开标题级线索与“detail 未取得/fallback 不可执行”的事实。
