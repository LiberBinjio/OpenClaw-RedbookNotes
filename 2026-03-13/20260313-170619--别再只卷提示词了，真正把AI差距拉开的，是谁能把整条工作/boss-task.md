...
4. 已复核同日较新 Boss 留痕，沿用已确认的趋势入口边界、归档结构约束、爆款改写要求与非公开发布边界。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 已确认 Topic 仍必须优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写进来源状态，并产出可交付给 Draft 的爆款拆解与改写方向。
7. 已确认 Draft 必须重写爆款模式，不得复制原句；最终产物仅用于预览、复审与手动发布，不执行公开发布。
8. 已确认本轮 archive package 必须统一为：`2026-03-13 / 20260313-170619+AI技能内容真正拉开差距的，不是提示词更花，而是有人先把流程跑成了可复用资产 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容真正拉开差距的，不是提示词更花，而是有人先把流程跑成了可复用资产`。
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-170619+AI技能内容真正拉开差距的，不是提示词更花，而是有人先把流程跑成了可复用资产 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容真正拉开差距的，不是提示词更花，而是有人先把流程跑成了可复用资产”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-170619` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会提问，转向能不能把选题、写稿、审稿、归档串成一条稳定复用的执行链路”，而不是泛泛讨论模型能力。
...
- 正文建议重点写清三段：为什么只把 AI 当聊天框会越来越难沉淀资产、为什么 workflow skill 更容易沉淀成可复用结果、怎样把选题 / 写稿 / 审稿 / 归档串成一条可重复执行的内容生产线。
...
- 主标题建议突出：`AI技能内容真正拉开差距的`。
- 副标题建议突出：`不是提示词更花，而是流程先被做成资产`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-170619`。
...
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-170619+AI技能内容真正拉开差距的，不是提示词更花，而是有人先把流程跑成了可复用资产`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-170619.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-170619`，避免 Topic / Draft / Cover / Review 落入旧时间戳归档包。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-170619.md`
- 发布动作：未执行
