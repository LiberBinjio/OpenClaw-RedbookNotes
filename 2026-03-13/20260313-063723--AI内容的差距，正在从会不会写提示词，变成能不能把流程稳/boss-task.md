...
4. 已复核上一轮稳定留痕 `Boss/BOSS-CYCLE-20260312-205815.md` 与 `Boss/BOSS-CYCLE-20260312-203019.md`，沿用其已确认的来源边界、archive package 约束与非公开发布边界。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 已再次尝试访问 `https://www.newrank.cn/search/trend/skill`，但当前 WebFetch 工具返回模型不支持错误，未能取得新的可验证详情。因此本轮仍只能沿用已留痕的稳定 Newrank 邻近线索，并明确记录详情缺失、fallback 不可用、以及在线抓取工具当前失败三重限制。
7. 已确认本轮 archive package 必须统一为：`2026-03-13 / 20260313-063723+AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程 / material files`。
8. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程`。
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-063723+AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程”的主选题。
...
- 本轮还必须补充说明：Boss 已尝试使用现有在线抓取能力，但 `WebFetch` 当前返回模型不支持错误，因此也不能把本轮写成已获取新的在线详情。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-063723` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
6. **来源限制说明**
   ...
   - 本轮尝试使用 `WebFetch` 访问 Newrank 时返回模型不支持错误，因此也不能把在线抓取写成已成功。
...
- 必须核验 Topic 是否如实说明本轮 `WebFetch` 访问 Newrank 失败，不能冒充新增在线证据。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-063723`。
...
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-063723+AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-063723.md`
...
## 当前判断
- 当前最大风险仍是来源留痕失真：`search/trend/skill` 精确详情没有稳定拿到，就必须诚实写限制；指定 fallback skill 当前不可用，也必须诚实写不可用；现有在线抓取失败，也必须诚实写失败。
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-063723`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-063723.md`
- 发布动作：未执行
