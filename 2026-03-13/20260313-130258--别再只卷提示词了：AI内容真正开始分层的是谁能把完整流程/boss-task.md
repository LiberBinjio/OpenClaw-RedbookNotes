...
4. 已复核上一轮 Boss 留痕与当前 Topic 任务要求，确认本轮仍需优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写入来源状态，并要求产出 `viral-copy breakdown`。
...
7. 已确认本轮 Boss 只新增一个任务文件，统一 archive package 需落到本轮时间戳 `20260313-130258`。
8. 已确认本轮 archive package 应统一为：`2026-03-13 / 20260313-130258+AI技能内容开始拼交付闭环，不拼提示词，能把流程跑完的人更容易留下结果 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始拼交付闭环，不拼提示词，能把流程跑完的人更容易留下结果`。
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-130258+AI技能内容开始拼交付闭环，不拼提示词，能把流程跑完的人更容易留下结果 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-130258` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-130258`。
- 必须核验最终产物仅用于预览、复审与手动发布，不自动公开发布。

## 归档要求
- 最终内容包必须能按 `date / timestamp+中文标题 / material files` 组织。
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-130258+AI技能内容开始拼交付闭环，不拼提示词，能把流程跑完的人更容易留下结果`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-130258.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-130258`，避免落入上一轮归档包。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-130258.md`
- 发布动作：未执行
