...
6. 已确认本轮 Topic 仍必须优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写进来源状态；但当前未取得可验证的 Newrank 深层详情正文，因此只能沿用已留痕的稳定 Newrank 邻近线索，并明确记录详情缺失与 fallback 不可用状态。
7. 已复核近邻 Boss 留痕 `Boss/BOSS-CYCLE-20260312-224939.md`，沿用其已确认的来源边界、archive package 约束、viral-copy breakdown 要求与非公开发布边界。
...
9. 已确认本轮 archive package 必须统一为：`2026-03-13 / 20260313-032816+AI技能开始拼交付，不拼提示词，谁能把流程跑完谁更容易被记住 / material files`。
...
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-032816+AI技能开始拼交付，不拼提示词，谁能把流程跑完谁更容易被记住 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-032816` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-032816`。
...
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-032816+AI技能开始拼交付，不拼提示词，谁能把流程跑完谁更容易被记住`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-032816.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-032816`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-032816.md`
- 发布动作：未执行
