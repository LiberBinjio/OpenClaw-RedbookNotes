...
4. 已复核上一轮稳定留痕 `Boss/BOSS-CYCLE-20260313-063723.md` 与当前 `Draft/@DRAFT.md`，沿用其已确认的来源边界、archive package 约束、viral-copy breakdown 要求与非公开发布边界。
5. 已确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 当前未拿到 `https://www.newrank.cn/search/trend/skill` 的可验证详情页正文；因此本轮仍只能延续已留痕的 Newrank 邻近趋势线索，并明确记录详情不可得。
7. 已确认本轮 archive package 必须统一为：`2026-03-13 / 20260313-070317+AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程 / material files`。
...
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-070317+AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程 / material files`。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-070317` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-070317`。
...
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-070317+AI技能内容开始真正分层：能被反复带走的，不是更长的提示词，而是能稳定复用的一整套流程`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-070317.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-070317`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-070317.md`
- 发布动作：未执行
