...
8. 已确认本轮 archive package 必须统一为：`2026-03-13 / 20260313-073227+AI技能内容开始拼的不是谁更会提问，而是谁先把一整条流程沉淀成可复用结果 / material files`。
9. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始拼的不是谁更会提问，而是谁先把一整条流程沉淀成可复用结果`。
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-073227+AI技能内容开始拼的不是谁更会提问，而是谁先把一整条流程沉淀成可复用结果 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始拼的不是谁更会提问，而是谁先把一整条流程沉淀成可复用结果”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-073227` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的竞争点正在从会不会写提示词，转向能不能把选题、写稿、审稿、归档这样的链路拆清并稳定复用”，而不是泛泛讨论模型参数。
...
- 主标题建议突出：`AI技能内容开始拼的不是谁更会提问`。
- 副标题建议突出：`而是谁先把流程沉淀成可复用结果`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-073227`。
...
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-073227+AI技能内容开始拼的不是谁更会提问，而是谁先把一整条流程沉淀成可复用结果`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-073227.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-073227`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-073227.md`
- 发布动作：未执行
