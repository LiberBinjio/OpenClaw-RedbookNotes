...
4. 已复核上一轮稳定 Boss 留痕与当前 Topic 角色要求，确认本轮仍需优先把 `https://www.newrank.cn/search/trend/skill` 作为趋势入口写入来源状态，并要求产出 `viral-copy breakdown` 与 `rewrite direction`。
5. 已再次确认当前会话可用 skill 只有 `simplify` 与 `claude-api`；任务要求中的 `yhslgg-arch/url-reader` 当前不可用，因此 fallback 要求存在，但本会话无法执行，不能伪造抓取结果。
6. 已确认 Draft 必须基于爆款结构改写，而不是复制句子；最终产物只用于预览、复审与手动发布，不执行公开发布。
7. 已确认本轮 Boss 只新增一个任务文件，统一 archive package 需落到本轮时间戳 `20260313-132908`。
8. 已确认本轮 archive package 应统一为：`2026-03-13 / 20260313-132908+AI技能内容开始拼可复用链路，不是谁更会写提示词，而是谁能把结果稳定交付 / material files`。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始拼可复用链路，不是谁更会写提示词，而是谁能把结果稳定交付`。
- 统一归档日期：`2026-03-13`。
- 统一归档包：`2026-03-13 / 20260313-132908+AI技能内容开始拼可复用链路，不是谁更会写提示词，而是谁能把结果稳定交付 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始拼可复用链路，不是谁更会写提示词，而是谁能把结果稳定交付”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-13`、`20260313-132908` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
- 在趋势表达上，要优先收口到“AI 的价值正在从会不会写 prompt，转向能不能把一条链路拆清、跑顺、反复复用”，而不是泛泛讨论模型能力。
...
- 正文建议重点写清三段：为什么只把 AI 当聊天框会越来越难沉淀价值、为什么 workflow skill 更容易沉淀成可复用结果、怎样把选题 / 写稿 / 审稿 / 归档串成一条可重复执行的内容生产线。
...
- 主标题建议突出：`AI技能内容开始拼可复用链路`。
- 副标题建议突出：`不是谁更会写提示词，而是谁能把结果稳定交付`。
...
- 必须核验 Topic 是否写清：`openclaw` 当前有稳定 Newrank 邻近线索；`openclaw skill` 有 `OpenClaw + Skill` 邻近文章线索但无 `search/trend/skill` 直连详情；`AI skill`、`workflow skill` 当前主要靠邻近趋势证据支撑；`copilot skill` 本轮检索证据不足。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-132908`。
- 必须核验最终产物仅用于预览、复审与手动发布，不自动公开发布。

## 归档要求
- 最终内容包必须能按 `date / timestamp+中文标题 / material files` 组织。
- `date` 层：`2026-03-13`。
- `timestamp+中文标题` 层：`20260313-132908+AI技能内容开始拼可复用链路，不是谁更会写提示词，而是谁能把结果稳定交付`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-132908.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-132908`，避免继续沿用上一轮时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-132908.md`
- 发布动作：未执行
