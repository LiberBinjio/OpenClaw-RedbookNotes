...
8. 已确认本轮 archive package 必须统一为：`2026-03-14 / 20260313-221006+AI技能内容开始卷的不是提示词长度，而是你能不能把一套流程稳定交付出来 / material files`。
9. 已确认本轮只准备预览、复审与手动发布所需内容，不执行任何公开发布动作。

## 本轮 Boss 任务
- 面向 `@ALL` 与 `@BOSS`：围绕单一中文主题继续收口，所有产物必须统一到同一个 archive package。
- 推荐统一主题：`AI技能内容开始卷的不是提示词长度，而是你能不能把一套流程稳定交付出来`。
- 统一归档日期：`2026-03-14`。
- 统一归档包：`2026-03-14 / 20260313-221006+AI技能内容开始卷的不是提示词长度，而是你能不能把一套流程稳定交付出来 / material files`。
...
- Topic 必须优先产出一个可直接支持“AI技能内容开始卷的不是提示词长度，而是你能不能把一套流程稳定交付出来”的主选题。
...
- 必须把 `date`、`timestamp`、`archive_title`、`archive_package` 全量更新到本轮：`2026-03-14`、`20260313-221006` 与统一中文标题，避免 Topic 与 Draft / Cover / Review 落入不同归档包。
...
- 主标题建议突出：`AI技能内容开始卷的不是提示词长度`。
- 副标题建议突出：`而是你能不能把流程稳定交付出来`。
...
- 必须核验 Topic / Draft / Cover / Review 的 archive timestamp 与 archive package 是否统一到 `20260313-221006`。
...
- `date` 层：`2026-03-14`。
- `timestamp+中文标题` 层：`20260313-221006+AI技能内容开始卷的不是提示词长度，而是你能不能把一套流程稳定交付出来`。
- `material files` 层建议保留：
  - `Boss/BOSS-CYCLE-20260313-221006.md`
...
- 当前次级风险是 archive package 漂移：本轮所有下游都必须统一切到 `20260313-221006`，避免 Topic / Draft / Cover / Review 继续沿用旧时间戳导致归档包不一致。
...
- 实际改动：新增 `Boss/BOSS-CYCLE-20260313-221006.md`
- 发布动作：未执行
